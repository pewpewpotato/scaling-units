...
variable "core_subnet_prefix" {
  description = "The address prefix for the core subnet in CIDR notation"
  type        = string

  validation {
    condition     = can(cidrhost(var.core_subnet_prefix, 0))
    error_message = "The core_subnet_prefix must be a valid CIDR block."
  }
}
